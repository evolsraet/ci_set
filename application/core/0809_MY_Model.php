...
class MY_Model extends CI_Model
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->qb_functions = (object) $this->qb_functions;
	...
	public function paging($rows_per_page, $total_rows = NULL, $page_number = 1) {
		...
		$this->qb_functions->limit[] = array($rows_per_page, $offset);
	...
	*/
	...
	protected function _qb_do() {
		foreach( (array) $this->qb_functions as $function => $function_content ) :
			foreach( (array) $function_content as $row ) :
				switch ( count($row) ) {
					case 0:
						$this->_database->{$function}();
						break;
					case 1:
						$this->_database->{$function}( $row[0] );
						break;
					case 2:
						$this->_database->{$function}( $row[0], $row[1] );
						break;
					case 3:
						$this->_database->{$function}( $row[0], $row[1], $row[2] );
						break;
					case 4:
						$this->_database->{$function}( $row[0], $row[1], $row[2], $row[3] );
						break;
					case 5:
						$this->_database->{$function}( $row[0], $row[1], $row[2], $row[3], $row[4] );
						break;
				}
			endforeach;
		endforeach;
	...
	protected function _qb_reset( $force_reset = FALSE ) {
		if( !$this->qb_cache || $force_reset ) {
			foreach( (array) $this->qb_functions as $function => $function_content ) :
				$this->qb_functions->{$function} = array();
			endforeach;
			$this->qb_cache = FALSE;
		}
	}
	...
	protected function qb_where_count() {
		return
			count($this->qb_functions->where)
			+ count($this->qb_functions->where_in)
			+ count($this->qb_functions->like);
			+ count($this->qb_functions->limit);
	}

	protected function qb_set_count() {
		return count($this->qb_functions->set);
	}
	...
	// 쿼리빌더 펑션들 - 추후 인보크로 교체 고려
		...
		public function group_start() {
			...
			$this->qb_functions->{__FUNCTION__}[] = $args;
			return $this;
		}
