...
class MY_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->_created_at_field = $this->prefix.'created_at';
        $this->_updated_at_field = $this->prefix.'updated_at';
        $this->_deleted_at_field = $this->prefix.'deleted_at';
    ...
    public function _prep_before_write($data)
    {
        ...
        if($multi===FALSE)
        {
            foreach ( is_array($data_as_array) ? $data_as_array : array() as $field => $value)
            {
        ...
        {
            foreach ( is_array($data_as_array) ? $data_as_array : array() as $key => $row)
            {
     ...
     * @return $this
     */
    public function where($field_or_array = NULL, $operator_or_value = FALSE, $value = FALSE, $with_or = FALSE, $with_not = FALSE, $custom_string = FALSE)
    {
        ...
        elseif( $field_or_array === null  ) {
            $this->_database->{$where_or}($field_or_array, $operator_or_value);
        }
        elseif(is_numeric($field_or_array))
        {
            $this->_database->{$where_or}(array($this->table.'.'.$this->primary_key => $field_or_array));
        }
        elseif(is_array($field_or_array) && !is_not_false($operator_or_value))
        {
            // 스트링 추가 가능토록
            foreach( $field_or_array as $key => $row ) :
                if( is_numeric($key) ) :    // 키가 숫자면 (배열 기본), 내용을 텍스트로 간주한다
                    $this->_database->where($row);
                else :
                    $this->_database->where($key, $row);
                endif;
            endforeach;
        }
        elseif(!is_not_false($value) && isset($field_or_array) && is_not_false($operator_or_value) && !is_array($operator_or_value))
        {
            $this->_database->{$where_or}(array($this->table.'.'.$field_or_array => $operator_or_value));
        }
        elseif(!is_not_false($value) && isset($field_or_array) && is_not_false($operator_or_value) && is_array($operator_or_value) && !is_array($field_or_array))
        {
            //echo $field_or_array;
            //exit;
            $this->_database->{$where_or.$not.'_in'}($this->table.'.'.$field_or_array, $operator_or_value);
        }
        elseif(isset($field_or_array) && is_not_false($operator_or_value) && is_not_false($value))
        {
    ...
    {
        if(!empty($this->before_delete) || !empty($this->before_soft_delete) || !empty($this->after_delete) || !empty($this->after_soft_delete) || ($this->soft_delete === TRUE))
        {
            $to_update = array();
            if(isset($where))
            {
     ...
     * Forces the delete of a row if soft_delete is enabled
     * @param null $where
     * @return bool
     */
    public function force_delete($where = NULL)
    ...
    private function _fetch_table()
    {
        if (!isset($this->table))
        {
            $this->table = $this->_database->dbprefix(preg_replace('/(_m|_model)?$/', '', strtolower(get_class($this))));
