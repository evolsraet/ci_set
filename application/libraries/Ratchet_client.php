...
class Ratchet_client
{
    ...
    public function __construct(array $config = array())
    {
        ...
        // $this->CI->load->helper('ratchet');

        // Define the config vars
        $this->config = (!empty($config)) ? $config : $this->CI->config->config;

        // kmh_print($this->config);
        // die();
...
class Server implements MessageComponentInterface
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        output('info', print_r($this->CI->ratchet_client->config['ratchet_client'],true) );
    ...
    public function onMessage(ConnectionInterface $client, $message)
    {
        ...
        output('info', '$this->subscribers,true');
        output('info', print_r($this->subscribers,true));
        ...
        if (!empty($datas->user_id) && $datas->user_id !== $client->resourceId) {
            ...
            if (!empty($this->CI->ratchet_client->callback['auth']) && empty($client->subscriber_id)) {
                ...
                output('error', '$auth ('.$auth.') - '.$client->resourceId);

                // Verify authentication
                // if (empty($auth) or !is_integer($auth)) {
                if (empty($auth)) {
                    output('error', '$auth ('.$auth.')');
                    output('error', 'empty($auth) ('.empty($auth).')');
                    // output('error', '!is_integer($auth) ('.!is_integer($auth).')');
                ...
                if ($this->CI->ratchet_client->debug) {
                    ...
                    output('info', 'Client ('.$client->resourceId.') try \''.$message);
                }
            }
        }
        ...
        if (!empty($message)) {
            ...
            foreach ($this->clients as $user) {
                ...
                output('error', print_r($user->subscriber_id,true) );
                output('error', print_r((array)$datas->recipient_id,true) );
                output('error', print_r(in_array($user->subscriber_id, (array)$datas->recipient_id ),true) );

                // Broadcast to single user
                if (!empty($datas->recipient_id)) {
                    if ( in_array($user->subscriber_id, (array)$datas->recipient_id ) ) {
    ...
    protected function send_message($user = array(), $message = array(), $client = array())
    {
        ...
        if ($this->CI->ratchet_client->debug) {
            ...
            output('info', 'Client ('.$client->resourceId.') send to ('.$user->resourceId.')');
        }
    }
}
